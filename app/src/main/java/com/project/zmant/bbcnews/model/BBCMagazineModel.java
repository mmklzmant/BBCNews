...
/**
 * @author zmant 2016/12/16 16:59
 * @classname BBCMagazineModel
 * @description BBC模块 FragmentMagazine Model层
 */

public class BBCMagazineModel {
    private BBCMagazinePresenter mPresenter;
    public BBCMagazineModel(BBCMagazinePresenter presenter)
    {
    ...
    private ArrayList<BBCCardViewBean> parseHtml(String html) {
        ...
        datas = mergeMagazine(titles, urls, imgurls, times, locations);
        return datas;
    }
     ...
     */
    private ArrayList<BBCCardViewBean> mergeMagazine(ArrayList<String> titles,ArrayList<String> urls,
                                                     ArrayList<String> imgurls, ArrayList<String> times, ArrayList<String> locations)
    {
        ...
        for(int i = 0; (i < titles.size() - 6); i++)
        {
            ...
            else
            {
                ...
                {
                    bean.setLocation(locations.get(i-4));
                }
                else
                {
                    bean.setLocation(locations.get(i-8));
                }

            }
            if(bean.getLocation().equals(""))
            {
                bean.setLocation("Magazine");
            }
            datas.add(bean);
        }
        return datas;
